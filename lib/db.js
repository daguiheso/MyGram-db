...
class Db {
  ...
  connect (cb) {
    ...
    let setup = co.wrap(function * () {
      ...
      if (dbTables.indexOf('images') === -1) {
        ...
        // crear indice userId pero que permita varios registros con el mismo indice
  ...
  saveImage (image, cb) {
    ...
    let tasks = co.wrap(function * () {
       ...
       * actualizamos objeto image con nuevo publicId despues de saber id
       *
  ...
  likeImage (id, cb) {
    ...
    // referencia a db
    let db = this.db
    // referencia a getImage para ser usada dentro de function generator
  ...
  getImage (id, cb) {
    ...
    let tasks = co.wrap(function * () {
      ...
      // si db no retorna la imagen
      if (!image) {
        // definiendo la palabra not found que espera el expection en los test
  ...
  getUser (username, cb) {
    ...
    let tasks = co.wrap(function * () {
      ...
      // esperando a que todos los indices esten creados
      yield r.db(db).table('users').indexWait().run(conn)
      // consulta a db para obtener user
      let users = yield r.db(db).table('users').getAll(username, {
        index: 'username'
      }).run(conn)

      // resultado por defecto es null
      let result = null

      /*
       * Utilizamos try catch ya que como utilizamos generator functions pues todos los errores que peudan ser lanzados
       * dentro de un yield,los puedo capturar con try catch
       */
      try {
        // intenta obtener el siguiente usuario y el primer valor con next()
  ...
  authenticate (username, password, cb) {
    ...
    /*
     * Utilizando metodo getUser ya creado. Como esta funcion getUser la vamos a utilizar
     * dentro de otra funcion (function generator * ()) sucede que this no lo vamos a tener
     * como tal en el function generator, asi que utilizamos .bind(this), para que este nuevo
     * metodo que estamos creando o la referencia del metodo sea tal cual la funcion getUser
     * que esta definida en la clase, con el fin de poderlo utilizar sin problemas dentro de task.
     *
     */
    ...
    let tasks = co.wrap(function * () {
      ...
      try {
        ...
        // en este caso no hacemos reject sino retornamos false
